...
# Credit: https://towardsdatascience.com/multi-class-classification-using-focal-loss-and-lightgbm-a6a6dec28872

class OneVsRestLightGBMWithCustomizedLoss:

    def __init__(self, loss, n_jobs=5):
        self.loss = loss
        self.n_jobs = n_jobs
    ...
    def _fit_binary(self, X, y, X_val, y_val, **fit_params):
        ...
        if len(unique_y) == 1:
            ...
            fit = lgb.Dataset(X, y, init_score=np.full_like(y, init_score_value, dtype=float), free_raw_data=False)
            if 'eval_set' in fit_params:
